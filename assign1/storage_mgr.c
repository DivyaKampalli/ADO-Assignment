...
{
	// Attempting to open the file in write and read mode and creating the file if it doesn't exist
	file = fopen(fileName, "w+");
	// Checking if the file opening was not successful
	if (!file)
	{
		// Returning an error code if the file opening was not successful
		return RC_FILE_NOT_FOUND;
	}
	else
	{
		// Allocating memory for a page-sized buffer and initialize it with null characters
		SM_PageHandle buffer_file = (SM_PageHandle)calloc(PAGE_SIZE, sizeof(char));
		memset(buffer_file, '\0', PAGE_SIZE);
		// Writing the buffer content which is filled with null characters to the file
		size_t no_characters = fwrite(buffer_file, sizeof(char), PAGE_SIZE, file);

		// Checking if number of characters is less than the page size
		if (no_characters < PAGE_SIZE)
		{
			// Returning an error code for the failed write operation
			return RC_WRITE_FAILED;
		}
		// Freeing the allocated memeory before returning the error code
		free(buffer_file);
		// Closing the file
		fclose(file);
	}
...
{
	// Checking if the file handler is initialized
	if (!checkFileHandlerIsInit(file_handler))
	{
...
{
	// Indicator for file not present
	int isNotPresent = -1;
	// Indicator for file present
	int isPresent = 0;
	// Intiailizing Placeholder for potential open file check
	int *openFile;
...
RC writeBlock(int page_number, SM_FileHandle *file_handler, SM_PageHandle pageData)
{
	// Checking if the specified page number is less than 0
	if (page_number < 0)
	{
		return RC_WRITE_FAILED;
	}
	// Checking if the page number is greater than total number of pages
	if (page_number > file_handler->totalNumPages)
	{
		return RC_WRITE_FAILED;
	}
	// Attempting to open the file in read and write mode
	file = fopen(file_handler->fileName, "r+");
	// Checking if the file handler is properly intialized
	if (!file_handler)
	{
    ...
    // Attempting to set the file position indicator to the beginning of the specified page
    if (fseek(file, (PAGE_SIZE * page_number), SEEK_SET) != 0) {
        // Error setting the file position indicator
        fclose(file);
        return RC_WRITE_FAILED;
    }

    // Writing the page data to the file
    size_t bytesWritten = fwrite(pageData, sizeof(char), PAGE_SIZE, file);
    ...
    // Updating the current page position in the file handler
    file_handler->curPagePos = page_number;

    // Moving the file position indicator to the end of the file to determine the total number of pages
    if (fseek(file, 0, SEEK_END) != 0) {
        // Error moving the file position indicator
        fclose(file);
        return RC_WRITE_FAILED;
    }

    // Updating the total number of pages in the file handler based on the file size and page size
    long fileSize = ftell(file);
    if (fileSize < 0 || fileSize % PAGE_SIZE != 0) {
        // Invalid file size or page size
        fclose(file);
        return RC_WRITE_FAILED;
    }
    
    file_handler->totalNumPages = fileSize / PAGE_SIZE;
...
// Function to write the current block
RC writeCurrentBlock(SM_FileHandle *file_handler, SM_PageHandle pageData)
{
	// Attempting to open the file in read and write mode
	file = fopen(file_handler->fileName, "r+");
	// Checking if the file handler is properly initialized
	...
	int current_position = file_handler->curPagePos;
	// Calculating the current page number
	int currentPageNumber = current_position / PAGE_SIZE;
	// Setting the first block to be written
	int first_block = 0;
	// Moving the file position indicator to the current position
	fseek(file, current_position, SEEK_SET);
	// Writing the page data to the file, considering it as part of the current block
	fwrite(pageData, first_block + 1, PAGE_SIZE, file);
	// Updating the current page position in the file handler
	file_handler->curPagePos = ftell(file);
	// Closing the file after writing
	fclose(file);
