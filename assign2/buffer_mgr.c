...
/*
	...
	- return : RC code
*/
RC initBufferPool(BM_BufferPool *const b_mgr, const char *const pageFN, 
		  const int numPages, ReplacementStrategy strategy, 
		  void *stratData)
{
	// Open the page file in read-only mode to check if it exists
    FILE *file = fopen(pageFN, "r");
    if (file == NULL) {
		// If the file does not exist, return an error code
